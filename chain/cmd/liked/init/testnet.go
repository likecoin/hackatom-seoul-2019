...
func TestnetFilesCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a LikeChain testnet",
...
Example:
	liked testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "liked",
		"Home directory of the node's daemon configuration",
	)
	cmd.Flags().String(flagNodeCliHome, "likecli",
...
func initTestnet(config *tmconfig.Config, cdc *codec.Codec) error {
	...
	likeConfig := srvconfig.DefaultConfig()
	likeConfig.MinGasPrices = viper.GetString(server.FlagMinGasPrices)
	...
	for i := 0; i < numValidators; i++ {
		...
		likeConfigFilePath := filepath.Join(nodeDir, "config/liked.toml")
		srvconfig.WriteConfigFile(likeConfigFilePath, likeConfig)
	}
